(function() {
    ...
    var loadFiles = function(example, files, sceneName) {
        var promise = OSG.osgDB.Registry
            .instance()
            .getReaderWriterForExtension('gltf')
            .readNodeURL(files);
    ...
    var Example = function() {
        ...
        this._aoUniforms = {
            ...
            uRadius2: osg.Uniform.createFloat1(
                this._config.radius * this._config.radius,
                'uRadius2'
            ),
            ...
            uIntensityDivRadius6: osg.Uniform.createFloat1(
                this._config.intensity,
                'uIntensityDivRadius6'
            ),
    ...
    Example.prototype = osg.objectInherit(ExampleOSGJS.prototype, {
        ...
        createDepthCameraRTT: function() {
            ...
            var rttDepth = this.createTexture(
                'rttDepth',
                Texture.NEAREST,
        ...
        buildMipmapPasses: function(downsampleFragment) {
            ...
            var uniforms = {
                ...
                uPreviousViewport: osg.Uniform.createFloat2(
        ...
        createComposer: function() {
            ...
            var rttAo = this.createTexture(
                'rttAoTexture',
                Texture.LINEAR,
                Texture.UNSIGNED_BYTE,
                rttSize
            );
            var rttAoHorizontalFilter = this.createTexture(
                'rttAoTextureHorizontal',
                Texture.LINEAR,
                Texture.UNSIGNED_BYTE,
                rttSize
            );
            ...
            var blurHorizontalPass = new osgUtil.Composer.Filter.Custom(
                blurFragment,
                horizontalBlurUniforms
            );
            ...
            var blurVerticalPass = new osgUtil.Composer.Filter.Custom(
                blurFragment,
                verticalBlurUniforms
            );
        ...
        initDatGUI: function() {
            ...
            sceneFolder
                .add(this._config, 'scale', 1.0, 1500.0)
                .onChange(this.updateScale.bind(this));
            sceneFolder
                .add(this._config, 'scene', this._modelList)
                .onChange(this.updateScene.bind(this, this._config.scene));

            ssaoFolder
                .add(this._config, 'radius', radiusBounds[0], radiusBounds[1])
                .onChange(this.updateRadius.bind(this));
            ssaoFolder
                .add(this._config, 'bias', biasBounds[0], biasBounds[1])
                .onChange(this.updateBias.bind(this));
            ssaoFolder
                .add(this._config, 'intensity', intensityBounds[0], intensityBounds[1])
                .onChange(this.updateIntensity.bind(this));

            blurFolder
                .add(this._config, 'crispness', 0.1, 2.5)
        ...
        bindProjectionUpdateCallback: function(camera) {
            ...
            camera.setClampProjectionMatrixCallback(function(
                m,
                computedNear,
                computedFar,
                nearFarRatio
        ...
        run: function() {
            ...
            this.readShaders().then(function() {
                ...
                self._viewer
                    .getManipulator()
                    .computeHomePosition(OSG.osgGA.Manipulator.COMPUTE_HOME_USING_SPHERE);
            });
        },
