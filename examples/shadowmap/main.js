    ...
    Example.prototype = osg.objectInherit( ExampleOSGJS.prototype, {
        ...
        updateLightsEnable: function () {
            ...
            if ( this._lights.length !== numLights ) {
                ...
                // remove all lights
                for ( var i = 0; i < l; i++ ) {
                    this._lightAndShadowScene.removeShadowTechnique( this._shadowTechnique[ i ] );

                    group.removeChild( this._lightsMatrix[ i ] );
                    group.removeChild( this._debugLights[ i ] );
                }
        ...
        updateShadowStatic: function () {
            ...
            // remove all lights
            for ( var i = 0; i < l; i++ ) {
                var st = this._shadowTechnique[ i ];
        ...
        updateLightsAmbient: function () {
            ...
            for ( var i = 0; i < l; i++ ) {
                this._lights[ i ].setAmbient( [ val, val, val, 1.0 ] );
            }
                ...
                case 'Point':
                    {
                        ...
                        l = this._lights.length;
                        for ( i = 0; i < l; i++ ) {
                            this._lights[ i ].setLightAsPoint();
                        }
                        break;
                    }
                case 'Directional':
                    {
                        ...
                        l = this._lights.length;
                        for ( i = 0; i < l; i++ ) {
                            this._lights[ i ].setLightAsDirection();
                        }
                        break;
                    }
                }
        ...
        updateShadowFormat: function () {
            ...
            if ( this._previousTextureType !== texType ) {
                var l = this._lights.length;
                for ( var i = 0; i < l; i++ ) {
                    var shadowSettings = this._shadowSettings[ i ];
        ...
        updateShadowMapSize: function () {
            ...
            if ( this._previousTextureSize !== mapsize ) {
                ...
                var l = this._lights.length;
                for ( var i = 0; i < l; i++ ) {
                    this._shadowSettings[ i ].setTextureSize( mapsize );
                }
        ...
        updateFov: function () {
            ...
            if ( this._previousFov !== this._config.fov ) {
                ...
                var l = this._lights.length;
                for ( var i = 0; i < l; i++ ) {
                    this._lights[ i ].setSpotCutoff( this._config.spotCutoff );
                }
                this._previousFov = this._config.fov;
            }
        ...
        updateShadow: function () {
            ...
            if ( this._updateRTT || ( this._previousRTT === false && this._config.debugRTT ) ) {
                ...
                if ( this._config.atlas ) {
                    ...
                    for ( i = 0; i < l; i++ ) {
                        shadowMap = this._shadowTechnique[ i ];
                        this._RTT.push( shadowMap.getTexture() );
                    }
                }
            ...
            if ( !this._config.atlas ) {
                for ( i = 0; i < l; i++ ) {
                    shadowMap = this._shadowTechnique[ i ];
                    shadowMap.setShadowSettings( this._shadowSettings[ i ] );
                    shadowMap.setDebug( this._config.debugBounds );
                }
            }

            // Iterate over all controllers
            for ( var k in this._gui.__controllers ) {
                this._gui.__controllers[ k ].updateDisplay();
            }
