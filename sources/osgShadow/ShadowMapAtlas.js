    ...
    MACROUTILS.objectInherit(ShadowTechnique.prototype, {
        ...
        */
        ...
        recomputeViewports: function() {
            ...
            var viewDivide = numViews > 1 ? 2 * Math.ceil(numViews / 2) : 1;
            mapSize = this._textureSize / Math.sqrt(viewDivide);

            var numShadowWidth = this._textureSize / mapSize;
            var numShadowHeight = this._textureSize / mapSize;

            for (var i = 0; i < numViews; i++) {
                var shadowMap = this._shadowMaps[i];

                var y = mapSize * (i % numShadowWidth);
                var x = mapSize * Math.floor(i / numShadowHeight);

                if (this._viewportDimension.length <= i) {
                    this._viewportDimension.push(vec4.fromValues(x, y, mapSize, mapSize));
                }
                {
                    this._viewportDimension[i] = vec4.set(
